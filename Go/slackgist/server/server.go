...
func ListenAndServeGRPC(address string) error {
	...
	s := Server{}
	api.RegisterSlackgistServer(grpcServer, &s)
...
func ListenAndServeREST(restAddress, grpcAddress string) error {
	...
	err := api.RegisterSlackgistHandlerFromEndpoint(
		ctx,
		mux,
		grpcAddress,
		opts,
...
func ListenAndServeUI(uiAddress string) error {
	return http.ListenAndServe(uiAddress, slackgist.CreateUIHandler())
}
